...
impl Point {
    ...
    fn add_command(&self, command: &SubmarineCommand) -> Point {
        match command {
            SubmarineCommand::Forward(val) => Point {
                x: self.x + i64::from(*val),
                y: self.y,
            },
            SubmarineCommand::Down(val) => Point {
                x: self.x,
                y: self.y + i64::from(*val),
            },
            SubmarineCommand::Up(val) => Point {
                x: self.x,
                y: self.y - i64::from(*val),
            },
        }
    ...
    fn advance(&mut self, command: &SubmarineCommand) -> &mut SubmarineState {
        match command {
            SubmarineCommand::Forward(val) => {
                ...
                self.current_position = self.current_position.add(&Point {
                    x: i64::from(*val),
                    y: i64::from(y),
                });
            }
...
mod tests {
    ...
    fn test_point_add_command_from_0() {
        ...
        assert_eq!(
            initial_point.add_command(&SubmarineCommand::Down(5)),
            Point::new(0, 5)
        );
        assert_eq!(
            initial_point.add_command(&SubmarineCommand::Up(5)),
            Point::new(0, -5)
        );
        assert_eq!(
            initial_point.add_command(&SubmarineCommand::Forward(5)),
            Point::new(5, 0)
        );
    }
    ...
    fn test_submarine_command_new() {
        assert_eq!(
            SubmarineCommand::new("forward 8"),
